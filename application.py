...
def categoryItemsJSON(category_name):
    """JSON endpoint for all items in category."""
...
def profile():
    ...
    if 'username' not in login_session:
        ...
        return render_template(
            'profile.html',
            name=login_session['username'], id=login_session['user_id'],
            pic=login_session['picture'], categories=categories)
...
def editCategory(category_name):
    ...
    items = (
        session.query(Item)
        .filter_by(category_id=editedCategory.id)
        .all()
        )
    categories = session.query(Category).order_by(asc(Category.name))
    ...
    if editedCategory.user_id != login_session['user_id']:
        flash('You are not authorized to edit this category. \
            You must be the category owner.')
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            return redirect(
                url_for('showItem', category_name=editedCategory.name)
                )
    else:
        return render_template(
            'category_edit_rev.html', category=editedCategory,
...
def deleteCategory(category_name):
    ...
    if categoryToDelete.user_id != login_session['user_id']:
        flash('You are not authorized to delete this category. \
            You must be the category owner.')
        return redirect(
            url_for('showItem', category_name=categoryToDelete.name)
            )
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'category_delete_rev.html',
            category=categoryToDelete, items=items, categories=categories)
...
def showItem(category_name):
    ...
    return render_template(
        'items_rev.html', items=items, category=category,
        categories=categories, creator=login_session['username'],
...
def newItem(category_name):
    ...
    if category.user_id != login_session['user_id']:
        flash('You are not authorized to create new menu items for %s. \
            You must be the creator of the category.' % category.name)
...
# EDIT an ITEM
@app.route(
    '/catalog/<category_name>/item/<item_name>/edit',
    methods=['GET', 'POST']
    )
@login_required
def editItem(category_name, item_name):
    ...
    if editedItem.user_id != login_session['user_id']:
        flash('You are not authorized to edit this item. \
            You must be the creator of this category.')
        return redirect(url_for('showItem', category_name=category.name))
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'item_edit_rev.html', category=category,
            item=editedItem, categories=categories)


# DELETE an ITEM
@app.route(
    '/catalog/<category_name>/item/<item_name>/<int:item_id>/delete',
    methods=['GET', 'POST']
    )
@login_required
def deleteItem(category_name, item_name, item_id):
    ...
    if request.method == 'POST':
        ...
        return render_template(
            'item_delete_rev.html', item=itemToDelete,
            category=category, categories=categories)
...
def gconnect():
    ...
    if result['issued_to'] != CLIENT_ID:
        reponse = make_response(
            json.dumps("Token's client ID does not match app ID", 401)
            )
    ...
    # Added '.access_token' due to non serializable error
